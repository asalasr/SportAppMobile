...
import com.example.sportapp.repositories.RutasRepository
...
class RutasViewModel  (application: Application) :  AndroidViewModel(application) {
    ...
    private val _rutas = MutableLiveData<List<Rutas>>()
    ...
    val rutasvm: LiveData<List<Rutas>>
        get() = _rutas


    //Repository
    private var rutasRepositoryObject = RutasRepository(application)
    ...
    private fun refreshDataFromNetwork() {
        ...
        Log.i("RutasViewModel", "refreshData()")
        rutasRepositoryObject.refreshDataRutas({
            _rutas.postValue(it)
    ...
    class Factory(val app: Application) : ViewModelProvider.Factory {
        override fun <T : ViewModel?> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(RutasViewModel::class.java)) {
                ...
                return RutasViewModel(app) as T
            }
